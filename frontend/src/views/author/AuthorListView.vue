...
const form = reactive<Partial<Author>>({ author_name: '', country: '' })
const rules: FormRules = {
  author_name: [{ required: true, message: '请输入作者姓名', trigger: 'blur' }],
}

function openCreate() {
  dialogMode.value = 'create'
  Object.assign(form, { author_id: undefined, author_name: '', country: '' })
...
async function onSubmit() {
  ...
  try {
    const payload = { author_name: form.author_name, country: form.country || null }
    if (dialogMode.value === 'create') {
      await createAuthor(payload)
      ElMessage.success('作者新增成功')
    } else if (form.author_id) {
      await updateAuthor(form.author_id, payload)
      ElMessage.success('作者已更新')
    }
...
async function onDelete(row: Author) {
  try {
    await ElMessageBox.confirm(`确定删除作者「${row.author_name}」？若存在关联图书将无法删除。`, '删除确认', {
      type: 'warning',
  ...
  try {
    await deleteAuthor(row.author_id)
...
<template>
  <div class="page-wrapper">
    <PageHeader title="作者管理" subtitle="图书作者信息">
      <template #extra>
        <el-button v-if="canWrite()" type="primary" @click="openCreate">
          <el-icon><Plus /></el-icon>新增作者
        </el-button>
      </template>
    </PageHeader>
    ...
    >
      <el-table-column prop="author_id" label="#" width="70" />
      <el-table-column prop="author_name" label="姓名" min-width="180" />
    ...
    <el-dialog v-model="dialogVisible" :title="dialogMode === 'create' ? '新增作者' : '编辑作者'" width="440" destroy-on-close>
      <el-form ref="formRef" :model="form" :rules="rules" label-width="80px">
        <el-form-item label="姓名" prop="author_name"><el-input v-model="form.author_name" /></el-form-item>
