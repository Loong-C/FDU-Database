...
本文档严格依据当前仓库内容编写，主要参考范围包括 `README.md`、`backend/` 后端代码、`frontend/` 前端代码、`sql/` 建库脚本与 CSV 数据、`docs/` 既有说明和接口测试文档。本文档作为系统需求分析、数据库设计、功能设计、模块划分、系统实现、代码文件说明以及安装、使用、测试说明的统一入口。
...
- 复杂促销、优惠券和价格策略引擎。
- 物流配送和仓储调拨细节。
- 销售后自动增加会员积分。前端开单页会展示预计积分，但后端 `sales/services.py` 当前只创建销售记录并扣减库存。
- 完整审计日志和操作流水。
...
flowchart LR
    Browser["浏览器 / 用户"]
    Vue["Vue 3 前端<br/>Vite + Element Plus + ECharts"]
    API["Django REST API<br/>JWT + DRF"]
    MySQL["MySQL 8<br/>业务表 + Django 表"]
    SQL["sql/*.sql + sql/data/*.csv"]

    Browser --> Vue
    Vue -- "Axios /api/v1" --> API
    API -- "PyMySQL" --> MySQL
    SQL -- "bootstrap_business_db --seed --views" --> MySQL
```
...
erDiagram
    ...
    STORE ||--o{ INVENTORY : owns
    PRODUCT ||--o{ INVENTORY : stocked_as
    SUPPLIER ||--o{ PURCHASE_ORDER : receives
    STORE ||--o{ PURCHASE_ORDER : orders_for
    PURCHASE_ORDER ||--|{ PURCHASE_ORDER_ITEM : contains
    PURCHASE_ORDER ||--o{ STOCK_IN : inbound_source
    STOCK_IN ||--|{ STOCK_IN_ITEM : contains
    PRODUCT ||--o{ PURCHASE_ORDER_ITEM : purchased_as
    PRODUCT ||--o{ STOCK_IN_ITEM : inbound_as
...
sequenceDiagram
    participant U as 用户
    participant F as 前端销售页
    participant A as 后端 Sales API
    participant I as Inventory Service
    participant DB as MySQL

    U->>F: 选择门店、搜索商品、加入购物车
    F->>A: POST /api/v1/sales
    A->>DB: 查询商品当前售价
    A->>A: 计算明细金额、总额、实付金额
    A->>I: 按门店和商品扣减库存
    I->>DB: SELECT FOR UPDATE 锁定 inventory
    I->>DB: 校验库存并保存新库存
    A->>DB: 创建 sale 与 sale_item
    A-->>F: 返回销售单详情
```
...
如只使用线上系统，可跳过本小节。需要在本地开发、测试或复现部署时，建议环境如下：
...
### 10.5 初始化数据库（本地开发或重建演示库可选）

本项目线上数据库已经完成初始化并可直接使用。只有在本地开发、测试或需要重建演示库时，才需要确认 MySQL 服务已启动后执行：
...
当前仓库对应系统已经部署在 `https://linkukai.com/bookstore/`，本章主要说明后续更新、重新部署或迁移部署时的构建方式。
...
该脚本不在当前仓库中，但 README 描述它会执行拉取代码、安装依赖、执行 migrations、刷新演示账号、构建前端、重启 Gunicorn、检查并重载 Nginx 等步骤。
...
后续若继续扩展，可优先考虑：统一前后端权限矩阵、补充会员积分自动结算、增加退货退款流程、补充更多采购状态流转控制、将 SQL 设计层 RBAC 与 Django 运行时用户权限完全打通。
